...
def get_wmt_enfr_train_set(directory):
  ...
  train_path = os.path.join(directory, "training_set")
  if not (gfile.Exists(train_path +".to") and gfile.Exists(train_path +".from")):
...
def get_wmt_enfr_dev_set(directory):
  """Download the WMT en-fr training corpus to directory unless it's there."""
  dev_name = "testing_set"
  dev_path = os.path.join(directory, dev_name)
  if not (gfile.Exists(dev_path + ".to") and gfile.Exists(dev_path + ".from")):
...
def nltk_tokenizer(sentence):
  """NLTK tokenizer added on 2017.04.29,
                  advised on 2017.05.02"""
  # Do not mess with encoding format (WILL BREAK DOWN, I GUARANTEE YOU).
  return nltk.word_tokenize(unicode(sentence, "utf-8"))
...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size,
                      tokenizer=nltk_tokenizer, normalize_digits=True):
  ...
  if not gfile.Exists(vocabulary_path):
    ...
    with gfile.GFile(data_path, mode="rb") as f:
      ...
      for line in f:
        ...
        tokens = tokenizer(line) if tokenizer(line) else basic_tokenizer(line)
        for w in tokens:
...
def sentence_to_token_ids(sentence, vocabulary,
  ...
  # Do not mess with encoding format (WILL BREAK DOWN, I GUARANTEE YOU).
  return [vocabulary.get(_DIGIT_RE.sub(b"0", w.encode("utf-8")), UNK_ID) for w in words]
...
def prepare_wmt_data(data_dir, en_vocabulary_size, fr_vocabulary_size, tokenizer=nltk_tokenizer):
  ...
  from_train_path = train_path + ".from"
  to_train_path = train_path + ".to"
  from_dev_path = dev_path + ".from"
  to_dev_path = dev_path + ".to"
...
def prepare_data(data_dir, from_train_path, to_train_path, from_dev_path, to_dev_path, from_vocabulary_size,
                 to_vocabulary_size, tokenizer=nltk_tokenizer):
